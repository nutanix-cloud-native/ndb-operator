...
var _ = Describe("Test Secret Utility", func() {
	Context("util.get_data_from_secret test", func() {
		...
		It("should get the data", func() {
			...
			// username
			data, err := util.GetDataFromSecret(ctx, k8sClient, ndbSecretName, namespaceName, ndbv1alpha1.NDB_PARAM_USERNAME)
			Expect(err).To(Not(HaveOccurred()))
			Expect(data).To(Equal(b64.StdEncoding.EncodeToString([]byte(username))))
			// password
			data, err = util.GetDataFromSecret(ctx, k8sClient, ndbSecretName, namespaceName, ndbv1alpha1.NDB_PARAM_PASSWORD)
			Expect(err).To(Not(HaveOccurred()))
			...
			// password
			data, err = util.GetDataFromSecret(ctx, k8sClient, instanceSecretName, namespaceName, ndbv1alpha1.NDB_PARAM_PASSWORD)
			Expect(err).To(Not(HaveOccurred()))
			Expect(data).To(Equal(b64.StdEncoding.EncodeToString([]byte(password))))
			// ssh key
			data, err = util.GetDataFromSecret(ctx, k8sClient, instanceSecretName, namespaceName, ndbv1alpha1.NDB_PARAM_SSH_PUBLIC_KEY)
			Expect(err).To(Not(HaveOccurred()))
			...
			_, err = util.GetDataFromSecret(ctx, k8sClient, ndbSecretName, namespaceName, ndbv1alpha1.NDB_PARAM_USERNAME)
			Expect(err).To(HaveOccurred())
			_, err = util.GetDataFromSecret(ctx, k8sClient, ndbSecretName, namespaceName, ndbv1alpha1.NDB_PARAM_PASSWORD)
			Expect(err).To(HaveOccurred())
			_, err = util.GetDataFromSecret(ctx, k8sClient, instanceSecretName, namespaceName, ndbv1alpha1.NDB_PARAM_PASSWORD)
			Expect(err).To(HaveOccurred())
			_, err = util.GetDataFromSecret(ctx, k8sClient, instanceSecretName, namespaceName, ndbv1alpha1.NDB_PARAM_SSH_PUBLIC_KEY)
			Expect(err).To(HaveOccurred())
		})
	})
})
