...
// MongoDBCloneSingleInstanceTestSuite is a test suite struct that embeds testify's suite.Suite
type MongoDBCloneSingleInstanceTestSuite struct {
	suite.Suite
	ctx               context.Context
...
func (suite *MongoDBCloneSingleInstanceTestSuite) SetupSuite() {
	var err error
	...
	logger, err := util.SetupLogger(fmt.Sprintf("%s/mongo-si_test.log", automation.CLONING_LOG_PATH))
	if err != nil {
		suite.T().FailNow()
	}
...
func (suite *MongoDBCloneSingleInstanceTestSuite) TearDownSuite() {
	var err error
...
func (suite *MongoDBCloneSingleInstanceTestSuite) BeforeTest(suiteName, testName string) {
...
func (suite *MongoDBCloneSingleInstanceTestSuite) AfterTest(suiteName, testName string) {
	util.GetLogger(suite.ctx).Printf("******************** END TEST %s %s ********************\n", suiteName, testName)
}

// Tests if provisioning is succesful by checking if database status is 'READY'
func (suite *MongoDBCloneSingleInstanceTestSuite) TestProvisioningSuccess() {
	logger := util.GetLogger(suite.ctx)
...
func (suite *MongoDBCloneSingleInstanceTestSuite) TestAppConnectivity() {
	logger := util.GetLogger(suite.ctx)

	resp, err := util.GetAppResponse(suite.ctx, suite.clientset, suite.setupTypes.AppPod, "3011")
	if err != nil {
...
func TestMongoDBCloneSingleInstanceTestSuite(t *testing.T) {
	suite.Run(t, new(MongoDBCloneSingleInstanceTestSuite))
}
