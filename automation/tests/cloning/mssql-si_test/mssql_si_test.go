...
// MSSQLCloneSingleInstanceTestSuite is a test suite struct that embeds testify's suite.Suite
type MSSQLCloneSingleInstanceTestSuite struct {
	suite.Suite
	ctx               context.Context
...
func (suite *MSSQLCloneSingleInstanceTestSuite) SetupSuite() {
	var err error
	...
	logger, err := util.SetupLogger(fmt.Sprintf("%s/mssql-si_test.log", automation.CLONING_LOG_PATH))
	if err != nil {
		suite.T().FailNow()
	}
...
func (suite *MSSQLCloneSingleInstanceTestSuite) TearDownSuite() {
	var err error
...
func (suite *MSSQLCloneSingleInstanceTestSuite) BeforeTest(suiteName, testName string) {
...
func (suite *MSSQLCloneSingleInstanceTestSuite) AfterTest(suiteName, testName string) {
	util.GetLogger(suite.ctx).Printf("******************** END TEST %s %s ********************\n", suiteName, testName)
}

// Tests if provisioning is succesful by checking if database status is 'READY'
func (suite *MSSQLCloneSingleInstanceTestSuite) TestProvisioningSuccess() {
	logger := util.GetLogger(suite.ctx)
...
func (suite *MSSQLCloneSingleInstanceTestSuite) TestAppConnectivity() {
	logger := util.GetLogger(suite.ctx)

	resp, err := util.GetAppResponse(suite.ctx, suite.clientset, suite.setupTypes.AppPod, "3014")
	if err != nil {
...
func TestMSSQLCloneSingleInstanceTestSuite(t *testing.T) {
	suite.Run(t, new(MSSQLCloneSingleInstanceTestSuite))
}
