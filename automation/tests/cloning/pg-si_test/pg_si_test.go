...
// PostgresCloneSingleInstanceTestSuite is a test suite struct that embeds testify's suite.Suite
type PostgresCloneSingleInstanceTestSuite struct {
	suite.Suite
	ctx               context.Context
...
func (suite *PostgresCloneSingleInstanceTestSuite) SetupSuite() {
	var err error
...
func (suite *PostgresCloneSingleInstanceTestSuite) TearDownSuite() {
	var err error
...
func (suite *PostgresCloneSingleInstanceTestSuite) BeforeTest(suiteName, testName string) {
...
func (suite *PostgresCloneSingleInstanceTestSuite) AfterTest(suiteName, testName string) {
	util.GetLogger(suite.ctx).Printf("******************** END TEST %s %s ********************\n", suiteName, testName)
}

// Tests if provisioning is succesful by checking if database status is 'READY'
func (suite *PostgresCloneSingleInstanceTestSuite) TestProvisioningSuccess() {
	logger := util.GetLogger(suite.ctx)
...
func (suite *PostgresCloneSingleInstanceTestSuite) TestAppConnectivity() {
	logger := util.GetLogger(suite.ctx)

	resp, err := util.GetAppResponse(suite.ctx, suite.clientset, suite.setupTypes.AppPod, "3010")
	if err != nil {
...
func TestPostgresCloneSingleInstanceTestSuite(t *testing.T) {
	suite.Run(t, new(PostgresCloneSingleInstanceTestSuite))
}
